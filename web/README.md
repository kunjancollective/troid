...
| `public/desk-preview.html`, `public/desk2.js` | `/desk-preview` | troid's next desk (design handoff 2026-09-24, section 4; ticker v2 handoff, sections E and F): the desk's own template rendered a second time with `desk2` on (`site_build.desk_preview`, `templates/partials/_desk2.html` and `_desk2.css`), so the sizing is the live desk's code and the live desk's page is unchanged byte for byte. English only, `noindex`, in no sitemap, linked from nowhere, until the owner has tried it on a phone and it replaces the home page. It adds: the account gauge (the logo's dot above two floors grown into the account, to scale; a bar pinned under the tape on a phone, a column from 1100 px), step cards (your account, your trade, your risk), the breaker ladder, the fee bar, and "How troid computed this" with the result's own numbers; the Asset field (firms.json `_asset_universe`), which changes only the hold-limit line and the availability note, each with its source; the entry chip (crypto from `/api/ticker`, gold, oil and stocks pointed to the tape); `?tvwidgetsymbol=` from the tape; and the stale-stop rule (a new entry that leaves the stop on the wrong side or more than 25% away clears it: "Set your stop"). Tests: `python web/test_desk_preview.py` (Chromium; the 84 states against the live desk's). |
| `public/pop.js` | — | The desk's glossary and its notes (design handoff 2026-09-24, sections 2 and 6; glossary v2): every field label and readout figure is a term (`data-tip`, dotted underline) whose note shows after 300 ms of hover, at once on keyboard focus, and toggles on a tap; a note with links (`data-pop`, the desk's "why these 3?") opens on a click or tap only. Esc or a click elsewhere closes either; a note whose figure the desk re-renders closes with it. Notes are built by `backtest/regions.glossary_html` from `glossary.*` in `web/i18n` and firms.json; the readout's carry a live "Now" line the desk writes from its own result. Every page with the calendar strip loads it for the strip's notes; it runs once however many times a page includes it. Tests: `python web/test_glossary_page.py` (Chromium). |
