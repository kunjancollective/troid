...
| `public/pop.js` | — | The "?" beside a term and the note it opens (design handoff 2026-09-24, sections 2 and 6): a tooltip (`data-tip`) shows after 300 ms of hover, at once on keyboard focus, and toggles on a tap; a note with links (`data-pop`, the desk's "why these 3?") opens on a click or tap only. Esc or a click elsewhere closes either. Content comes from `web/i18n` and firms.json through the templates, so it translates; the page's CSS draws it (`.pop`, `.qm`). |
