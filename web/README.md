...
| `api/ticker.js`, `public/ticker.js` | `/api/ticker` | The price strip under every page's header (design handoff 2026-09-24, section 3): BTC, ETH, SOL, XRP and BNB, last spot price and 24-hour change, from api.binance.us (USDT pairs, the feed troid's ledger runs on), fetched server-side and kept at the CDN for 15 s (`s-maxage=15, stale-while-revalidate=30`), so the browser talks only to troid.ai. The change is ▲/▼ in the page's dim ink, never green or red. Over 60 s old (the CDN's Age plus the exchange-to-server gap, never the visitor's clock) it greys and says "delayed"; a failure hides it with its space kept. On the desk a symbol's note offers "use as entry". Tests: `node web/test_ticker.js` (the function, a stubbed upstream) and `python web/test_ticker_page.py` (Chromium). |
