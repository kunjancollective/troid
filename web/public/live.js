/* troid's status light: the dot in the tr●id wordmark (site_build.mark). It ripples while troid's ledger is live,
   ...
   from the same file (site_build.status_now), so the dot is right on first paint, but not its time: this confirms or
   corrects the state, writes the label with its time, and the page's CSS fades any change. When status.json can't be read the dot holds still and says so. Checked on load,
