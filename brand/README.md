# troid logo — final

Mark: a signal dot above its floor. The dot is the status light — the droid is on. The
floor is the loss ceiling. The dot stays above the line; that is the product.

Wordmark: troid in IBM Plex Mono SemiBold with the o replaced by the dot. The dot is
nudged 5% of a cell to the right of geometric centre — an optical correction, because
the r carries its weight on the right and reads closer than it is.

| use | file |
|---|---|
| X / Reddit / YouTube / GitHub avatar | mark-400.png (or mark-1024.png where allowed) |
| X header 1500×500 | banner-x-1500x500.png |
| Reddit banner 1920×384 | banner-reddit-1920x384.png |
| YouTube banner 2560×1440 | banner-youtube-2560x1440.png |
| Site favicon | favicon.ico, mark-32.png |
| Apple touch icon | mark-180.png |
| Site header | lockup-transparent.png |
| Wordmark alone, dark / light / transparent | wordmark-*.png |
| Two-ceilings variant, for large in-page use | mark-2ceilings-*.png |

Colours: bg #070b12 · ink #e6edf5 · signal #4da3ff, halo at 18%.
Marks sit inside the centre 80% of the canvas; circle crops will not clip them.
Do not stretch, recolour, add text, or set on a gradient other than the brand's.
